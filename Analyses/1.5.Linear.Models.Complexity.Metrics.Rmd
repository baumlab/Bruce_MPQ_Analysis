...
# 1. VRM
## Fixed Effect Models {.tabset}
### Investigating categorical Hum_Dist Vs. various regions
...
4. VRM ~ year and region 4 (4 options (I only have 3 though): Vaskess Bay, BOW, Leeward)) --> Should perform similarly to region.3
...
Results:
- Region.3 and Region.4 are equal in terms of AIC, so can progress forward with either one
...
#AIC's for all
AIC(vrm5, vrm6, vrm7, vrm8, vrm9, vrm10,vrm11, vrm12, vrm13, vrm14, vrm15, vrm16) #All models using continuous human disturbance fit best, with lowest AIC = 12, 13 (delta AIC < 3 for all 11-16 models)

#So interaction between human disturbance and year not best fit when looking at it like this....


```

## Adding in site as a random effects {.tabset}
### Human disturbance interaction with year
```{r interactions}
