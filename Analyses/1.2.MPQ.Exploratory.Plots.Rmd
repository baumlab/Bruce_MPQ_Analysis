...
- Plot inidcates volumetric change at the two timepoints sampled, with an "overall" change from 2015 - 2019 category added

- Signficantly different volumetric change occured during the 2017-2019 timepoint from the other two plotted
...
- Larger variation in the intial years of the study 
...
- Lowered volumetric loss in the latter timepoint occured in both Very Low and Very High disturbance levels
...
- This is beleived to be the case due to a large storm in january 2015 destroying most of the more "complex" corals (primarily branching and foliose) and in removing those form the reef, left the less-complex, massive and submassive structure which is harder to erode and/or degrades slower
...
- Both site 5 and 37 are located in Vaskess bay is on the southern edge of the atoll (with the other VL sites in the BOW on a more north-eastern facing bay) 
...
#### 2.1 Plot size through time

- Plot size had been shown to vary over time in previous exploratory plots

- This issue has been fixed now by copying  previous plot areas in ArcMap and importing them into subsequent years plots before orienting them on consistent stake and benthic topogrphy
...
-Overarching theme for most metrics: After a large initial declines occured at the tail end/immediately following the mass mortality event (2016a) and then complexity stayed consistent through time
...
- Significant differences occured at the 1cm DEM, yet were lost in later DEM resolutions
...
- Above trend is less apparent and/or dissapears at larger DEM resolutions
...
####1cm#####
VRM.1 <- ggplot(mpq.1, aes(x = year, y=vrm)) + geom_boxplot(fill="deepskyblue") + theme_classic(base_size = 10) +facet_wrap(~DEM, scales = "fixed", ncol= 2)+ xlab("Year") + ylab("VRM") + geom_label(label= "p=2.81E-10", x= 3, y=0.120, label.size= 0.02, label.padding= unit(0.02, "lines"), fill= "grey", color="black") + ylim(0.02, 0.12)
...
- Trend of profile curvature shifting from more negative towards more positive values through time, indicating shifts in structure orientation to more inwardly concave (pos pro.curv values).

- I hypothesis this is due to erosion, as profile curvature deals with the orientation at the point of maximum slope and therefore that is where I beleive the flow of water and erosional debris would be moving the strongest. 
...
- This could again be due to erosional forces, as these heavily erode the face of the maximum slope, thereby leaving lips or ridges directly perpendicular to the flow.
...
- This section highlights variability across the disturbance gradient, with very low disturbance sites highing higher values across metrics than medium and very low disturbance sites (not statistically tested yet)
...
- Those intial losses at all disturbance levels are more pronounced with VRM than any other metric
...
- This section is to highlight differences betwen the regions that make up the Very Low disturbance level: Bay of Wrecks & Vaskes Bay.
...
- I beleive this is due to the different coral communities present at the two sites, so this will be worth looking at in more detail with the morphological digitization work. 
