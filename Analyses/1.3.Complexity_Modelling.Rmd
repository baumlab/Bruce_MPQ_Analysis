...
- Multivariate analyses: I first computed multiple NMDS's to indicate drivers and then followed that up with RDA's to determine statistical significance of those same drivers.

- Linear Modelling: I will use the knowledge gained from the multivariate analyses on what the "driving forces" of the trends we are seeing and use them as key components during the linear mixed-effect model fitting process, similar to Jenn Magel. 
...
#Assessing Coliniearity in the MPQ Complexity Metrics

```{r coliniearity}

mpq.1#Data 
env3a <- mpq.1[,9:14] #Data with only numeric values

## Create a bivariate plot 
source("panelutils.R")
op <- par(mfrow=c(1,1), pty = "s")
pairs(env3a, panel = panel.smooth, diag.panel = panel.hist, main = "Bivariate Plot with Histograms and Smooth Curves")
par(op) # Reset graphing window to default

```

# Fractal Dimension {.tabset}
...
- Fractal Dimension is calculated by the following equation:
...
C. BOW & Vaskes bay (both locations of VL disturbance) experience sharp declines between 2015 and 2016a. I hypothesize that the S.Lagoon might have as well, if not for the large macro algae bloom smothering the corals there and limiting ArcMaps ability to read the complexity of the underlying substrate.
...
- 2015 & 2017 are distinctly different, with no overlap, while 2019 acts as the "intermediary", encompassing bits of both ellipses
- VRM + live coral cover is driven by the 2015 values, while 2017 is driven by the dead coral, sand, and macro algae. 
...
I didn't create a data frame for these because I would just be modelling live coral colonies and non-coral colonies alone, as the dead categories weren't digitized by individual colony.



### 1c. Percent Cover examining changes in DEM scale with a different coding style 

- I computed this NMDS to see whether altering the DEM resolution that the complexity (environmental) components were used

- All mpq data is plotted three times: once as raw data, the other with MPQ data re-scaled to Z scores, the last with MPQ re-scaled + % cover data hellinger transformed.
...
- Stress is least at option 3 (with both data sets transformed/re-scaled)
...
B. Only VRM, Pro + Plan Curv significant; all morphologies significant

C. Only VRM, Pro + Plan Curv significant; all morphologies significant
...
##### C. MPQ Standardized & % Cover Hellinger Transformed
...
A. Profile Curv only significant complexity metric; all morphologies significant 

B. Profile Curv only significant complexity metric (Plan close); all morphologies significant 

C. Profile Curv only significant complexity metric (VRM + Plan are close); all morphologies significant 
...
A. No significant complexity metric (average curv + pro curv close); all morphologies significant 

B. No significant complexity metric; all morphologies significant 

C. No significant complexity metric; all morphologies significant 

Results: Standardization/re-scaling of the data frames didn't change significance, other than highlighting the differences between regions (specifically all regions VS N.Lagoon (Very low disturbance))
...
In this data frame, I have added the polygons previously tagged as live_massive_grooved and live_porities into the live_massive category. 
...
The colony count data is interesting, as we range from 0's into the hundreds of colonies. NMDS hate 0's, especially lots of them, so I added a constant (1) to all cells and therefore a value of 1 = 0 individuals were present. This allows NMDS to run well, and proportionally shouldn't affect the differences between the morphologies, right?
...
## 3. Full Data set Classification 
...
E & F. I am not 100% sure how to get the habitat volume data frame & % cover to align, so will need to guidance on how to statistically proceed here

G. Again S Lagoon (Med dist) and Vaskess (VL dist) group together, while BOW (VL dist) and N. Lagoon (VH dist) are unique to all others

H. 2015 and 2017 are unique from one another, with 2019 fitting between them
...
- MPQ (environmental) data has been re-scaled to Z scores 
...
#### E. Habitat Volume, % cover ~ Region

As Habitat Volume and fractal dimension are measured on different scales than the other complexity metrics, these have to be done separately for the NMDS

I don't understand how to alter the data frames so that the row lengths are the same (requirement for the NMDS to run). I will continue to work on this, but may need some direction as I currently have Percent Cover of each Coral Morphology = 80rows and Volumetric Change = 71 rows. 

```{r CloudCompare and percent cover by region for Full Dataset Classification, eval=FALSE, include=FALSE}
#Dataframes to use
dig <- read.csv("~/Desktop/GitHub/Bruce_MPQ_Analysis/Data/BaumLab_Digitization_Wide_PercentCover_Data_Clean.csv", header = TRUE, row.names = 1)
...
com3c <- decostand(com3c, "hellinger")


#Convert fractal dataframe to only years 2015, 2017, 2019
env3c <- subset(CC, select = c("volume.change"))
...
#Run MDS and envfit functions
dig.pc.mds.3c <- metaMDS(com3c, distance = "bray")
mpq.envfit.3c <- envfit(dig.pc.mds.3c, env3c, permutations = 999) # this fits environmental vectors
dig.spp.fit.3c <- envfit(dig.pc.mds.3c, com3c, permutations = 999) # this fits species vectors

#Stress of NMDS
dig.pc.mds.3c #0.1357


#### CREATE SITE SCORES DATAFRAME ###
#Determine the significant the different variables measured
mpq.envfit.3d #Fractal dimension is sig
dig.spp.fit.3d #Benthic Morphologies many are significant

#To plot the output from the mds using ggplot a new datasheet needs to be created which contains the x,y points for each site. You can do this by calling the scores of you mds.
site.scrs.3d <- as.data.frame(scores(dig.pc.mds.3d, display = "sites")) #save NMDS results into dataframe
site.scrs.3d <- cbind(site.scrs.3d, region = dig$region) #add grouping variable "Management" to dataframe
site.scrs.3d <- cbind(site.scrs.3d, year = dig$year) #add grouping variable of cluster grouping to dataframe
#site.scrs <- cbind(site.scrs, Site = rownames(site.scrs)) #add site names as variable if you want to display on plot
...
#A new dataset containing species data also needs to be made to look at species vectors.
#This is not necessary if you don’t want to show the species on the final graph.
spp.scrs.3d <- as.data.frame(scores(dig.spp.fit.3d, display = "vectors")) #save species intrinsic values into dataframe
spp.scrs.3d <- cbind(spp.scrs.3d, Morphologies = rownames(spp.scrs.3d)) #add species names to dataframe
spp.scrs.3d <- cbind(spp.scrs.3d, pval = dig.spp.fit.3d$vectors$pvals) #add pvalues to dataframe so you can select species which are significant
#spp.scrs<- cbind(spp.scrs, abrev = abbreviate(spp.scrs$Species, minlength = 6)) #abbreviate species names
sig.spp.scrs.3d <- subset(spp.scrs.3d, pval<=0.05) #subset data to show species significant at 0.05
head(spp.scrs.3d)

##### CREATE EXTRINSIC ENVIRONMENTAL VARIABLE DF #####
#To show environmental extrinsic variables another dataframe needs to be created
env.scores.mpq.3d <- as.data.frame(scores(mpq.envfit.3d, display = "vectors")) #extracts relevant scores from envifit
env.scores.mpq.3d <- cbind(env.scores.mpq.3d, env.variables = rownames(env.scores.mpq.3d)) #and then gives them their names
env.scores.mpq.3d <- cbind(env.scores.mpq.3d, pval = mpq.envfit.3d$vectors$pvals) # add pvalues to dataframe
sig.env.scrs.3d <- subset(env.scores.mpq.3d, pval<=0.05) #subset data to show variables significant at 0.05

head(env.scores.mpq.3d)

#Prepping plot to plot
nmds.plot.coarse.3d <- ggplot(site.scrs.3d, aes(x=NMDS1, y=NMDS2))+ #sets up the plot
  geom_point(aes(NMDS1, NMDS2, colour = factor(site.scrs.3d$region), shape = factor(site.scrs.3d$year)), size = 2)+
  coord_fixed()+
  theme_classic()+ 
  theme(panel.background = element_rect(fill = NA, colour = "black", size = 1, linetype = "solid"))+
  labs(colour = "Region", shape = "Year")+ # add legend labels for Management and Landuse
...
nmds.plot.coarse.3d + labs(title = "Basic ordination plot") #displays plot


#Adding vectors of significant species to plot
nmds.plot.coarse.3d+
  geom_segment(data = sig.spp.scrs.3d, aes(x = 0, xend=NMDS1, y=0, yend=NMDS2), arrow = arrow(length = unit(0.25, "cm")), colour = "grey10", lwd=0.3) + #add vector arrows of significant species
  ggrepel::geom_text_repel(data = sig.spp.scrs.3d, aes(x=NMDS1, y=NMDS2, label = Morphologies), cex = 3, direction = "both", segment.size = 0.25)+ #add labels for species, use ggrepel::geom_text_repel so that labels do not overlap
  labs(title = "Ordination with Coral Morphologies")


#Adding vectors of Significant Complexity Metrics
nmds.plot.coarse.3d+
  geom_segment(data = sig.env.scrs.3d, aes(x = 0, xend=NMDS1, y=0, yend=NMDS2), arrow = arrow(length = unit(0.25, "cm")), colour = "grey10", lwd=0.3) + #add vector arrows of significant env variables
  ggrepel::geom_text_repel(data = sig.env.scrs.3d, aes(x=NMDS1, y=NMDS2, label = env.variables), cex = 4, direction = "both", segment.size = 0.25)+ #add labels for env variables
  labs(title="Ordination with Complexity Metrics") 
...
#data for ellipse, in this case using the management factor
df_ell.dig.management.3d <- data.frame() #sets up a data frame before running the function.
for(g in levels(site.scrs.3d$region)){
  df_ell.dig.management.3d <- rbind(df_ell.dig.management.3d, cbind(as.data.frame(with(site.scrs.3d [site.scrs.3d$region==g,],
                                                         veganCovEllipse(cov.wt(cbind(NMDS1,NMDS2),wt=rep(1/length(NMDS1),length(NMDS1)))$cov,center=c(mean(NMDS1),mean(NMDS2))))) ,region=g))
}
 
# data for labelling the ellipse
NMDS.mean.dig.3a=aggregate(site.scrs.3d[ ,c("NMDS1", "NMDS2")], 
                         list(group = site.scrs.3d$region), mean)
 
# data for labelling the ellipse
NMDS.mean.3a=aggregate(site.scrs.3d[,c("NMDS1", "NMDS2")], 
                    list(group = site.scrs.3d$region), mean)

#Plot the elipses
nmds.plot.coarse.3d+ 
geom_path(data = df_ell.dig.management.3d, aes(x = NMDS1, y = NMDS2, group = region)) #this is the ellipse, seperate ones by region 

```

```{r CloudCompare and percent cover by region for Full Dataset Classification plot, eval=FALSE, include=FALSE}

#Plot all together with elipses
#Adding vectors of significant species to plot
nmds.plot.coarse.3d+
geom_path(data = df_ell.dig.management.3d, aes(x = NMDS1, y = NMDS2, colour = region)) +
  geom_segment(data = sig.spp.scrs.3d, aes(x = 0, xend=NMDS1, y=0, yend=NMDS2), arrow = arrow(length = unit(0.25, "cm")), colour = "grey10", lwd=0.3) + #add vector arrows of significant species
  ggrepel::geom_text_repel(data = sig.spp.scrs.3d, aes(x=NMDS1, y=NMDS2, label = Morphologies), cex = 3, direction = "both", segment.size = 0.25)+ #add labels for species, use ggrepel::geom_text_repel so that labels do not overlap
  labs(title = "Ordination with Coarse Coral Morphologies % Cover") + geom_segment(data = sig.env.scrs.3d, aes(x = 0, xend=NMDS1, y=0, yend=NMDS2), arrow = arrow(length = unit(0.25, "cm")), colour = "black", lwd=0.9) + ggrepel::geom_text_repel(data = sig.env.scrs.3d, aes(x=NMDS1, y=NMDS2, label = env.variables), cex = 4, direction = "both", segment.size = 0.25)

```


#### F. Habitat Volume, % cover ~ Year



#### H. Fractal Dimension, Hellinger % cover ~ Region 

```{r Fractal Dimension percent cover by region for Full Dataset Classification, include=FALSE}
#Dataframes to use
dig <- read.csv("~/Desktop/GitHub/Bruce_MPQ_Analysis/Data/BaumLab_Digitization_Wide_PercentCover_Data_Clean.csv", header = TRUE, row.names = 1)

frac <- read.csv("~/Desktop/GitHub/Bruce_MPQ_Analysis/Data/BaumLab_Fractal_Dimension_Data_Clean.csv", header = TRUE, row.names = 1)

#Change environment & community to a numeric value only df
com3d <- dig[ , 7:33]

#Hellinger Transform community data
com3d <- decostand(com3d, "hellinger")


#Convert fractal dataframe to only years 2015, 2017, 2019
env3d <- subset(frac, year == 2015 | year == 2017 | year == 2019)
env3d <- subset(env3d, select = c("fractal.dimension"))
...
#Run MDS and envfit functions
dig.pc.mds.3d <- metaMDS(com3d, distance = "bray")
mpq.envfit.3d <- envfit(dig.pc.mds.3d, env3d, permutations = 999) # this fits environmental vectors
dig.spp.fit.3d <- envfit(dig.pc.mds.3d, com3d, permutations = 999) # this fits species vectors

#Stress of NMDS
dig.pc.mds.3d #0.1231
...
```{r Fractal Dimension percent cover by region for Full Dataset Classification plot, echo=FALSE}
...
geom_path(data = df_ell.dig.management.3d, aes(x = NMDS1, y = NMDS2, colour = region)) +
  geom_segment(data = sig.spp.scrs.3d, aes(x = 0, xend=NMDS1, y=0, yend=NMDS2), arrow = arrow(length = unit(0.25, "cm")), colour = "grey10", lwd=0.3) + #add vector arrows of significant species
  ggrepel::geom_text_repel(data = sig.spp.scrs.3d, aes(x=NMDS1, y=NMDS2, label = Morphologies), cex = 3, direction = "both", segment.size = 0.25)+ #add labels for species, use ggrepel::geom_text_repel so that labels do not overlap
  labs(title = "Ordination with Coarse Coral Morphologies % Cover") + geom_segment(data = sig.env.scrs.3d, aes(x = 0, xend=NMDS1, y=0, yend=NMDS2), arrow = arrow(length = unit(0.25, "cm")), colour = "black", lwd=0.9) + ggrepel::geom_text_repel(data = sig.env.scrs.3d, aes(x=NMDS1, y=NMDS2, label = env.variables), cex = 4, direction = "both", segment.size = 0.25)+ annotate("text", x=-0.7, y=0.8, label= "stress = 0.123", size = 4)

```


#### I. Fractal Dimension, Hellinger % cover ~ Year
```{r Fractal Dimension percent cover  by year for Full Dataset Classification, include = FALSE}
#Prepping plot to plot
nmds.plot.coarse.3d <- ggplot(site.scrs.3d, aes(x=NMDS1, y=NMDS2))+ #sets up the plot
  geom_point(aes(NMDS1, NMDS2, colour = factor(site.scrs.3d$year), shape = factor(site.scrs.3d$region)), size = 2)+
  coord_fixed()+
  theme_classic()+ 
  theme(panel.background = element_rect(fill = NA, colour = "black", size = 1, linetype = "solid"))+
  labs(colour = "Year", shape = "Region")+ # add legend labels for Management and Landuse
...
#Add levels to the year category in the site scores df
levels(site.scrs.3d$region) 
levels(site.scrs.3d$year) 
site.scrs.3d$year <- factor(site.scrs.3d$year, levels = c("2015", "2017", "2019"))


#data for ellipse, in this case using the management factor
df_ell.dig.management.3dd <- data.frame() #sets up a data frame before running the function.
for(g in levels(site.scrs.3d$year)){
  df_ell.dig.management.3dd <- rbind(df_ell.dig.management.3dd, cbind(as.data.frame(with(site.scrs.3d [site.scrs.3d$year==g,],
                                                         veganCovEllipse(cov.wt(cbind(NMDS1,NMDS2),wt=rep(1/length(NMDS1),length(NMDS1)))$cov,center=c(mean(NMDS1),mean(NMDS2))))) ,year=g))
}
 

# data for labelling the ellipse
NMDS.mean.dig.3aa=aggregate(site.scrs.3d[ ,c("NMDS1", "NMDS2")], 
                         list(group = site.scrs.3d$year), mean)
 
# data for labelling the ellipse
NMDS.mean.3aa=aggregate(site.scrs.3d[,c("NMDS1", "NMDS2")], 
                    list(group = site.scrs.3d$year), mean)

#Plot the elipses
nmds.plot.coarse.3d+ 
geom_path(data = df_ell.dig.management.3dd, aes(x = NMDS1, y = NMDS2, colour = year)) #this is the ellipse, seperate ones by region 

```

```{r fractal dimension percent cover region for Full Dataset Classification plot, echo=FALSE}
#Plot all together with elipses
#Adding vectors of significant species to plot
nmds.plot.coarse.3d+
geom_path(data = df_ell.dig.management.3dd, aes(x = NMDS1, y = NMDS2, colour = year)) +
  geom_segment(data = sig.spp.scrs.3d, aes(x = 0, xend=NMDS1, y=0, yend=NMDS2), arrow = arrow(length = unit(0.25, "cm")), colour = "grey10", lwd=0.3) + #add vector arrows of significant species
  ggrepel::geom_text_repel(data = sig.spp.scrs.3d, aes(x=NMDS1, y=NMDS2, label = Morphologies), cex = 3, direction = "both", segment.size = 0.25)+ #add labels for species, use ggrepel::geom_text_repel so that labels do not overlap
  labs(title = "Ordination with Coarse Coral Morphologies % Cover") + geom_segment(data = sig.env.scrs.3d, aes(x = 0, xend=NMDS1, y=0, yend=NMDS2), arrow = arrow(length = unit(0.25, "cm")), colour = "black", lwd=0.9) + ggrepel::geom_text_repel(data = sig.env.scrs.3d, aes(x=NMDS1, y=NMDS2, label = env.variables), cex = 4, direction = "both", segment.size = 0.25)+ annotate("text", x=-0.7, y=0.8, label= "stress = 0.123", size = 4)
...
### 1. Percent cover for entire data frame {.tabset}

- 1st RDA Axis is significant, all other axes are not significant

- R2 value of 0.15 (15%)

- MPQ Complexity data was re-scaled to Z scores | % Cover data was hellinger transformed
...
#### Scaling 1
```{r  RDA percent cover scaling 1, include=FALSE}
#Load the data
dig <- read.csv("~/Desktop/GitHub/Bruce_MPQ_Analysis/Data/BaumLab_Digitization_Wide_PercentCover_Data_Clean.csv", header = TRUE, row.names = 1)
...
RDA 1 (65.8%) and 2 (6.2%) explain roughly 72% of the variance in the data, with an Adjusted R2 value backing that up of 69.1%. 

The eigenvalue of PC1 is quite small (0.024), indicating that it does not describe any important residual structure in the response data (mpq complexity metrics).

To describe this result in my discussion:
Looking at Scaling 1: Which red arrows are pulling things around in orthogonal/perpendictular to one another? Answer: VRM/Rug, Pro/Plan Curv, and average curvature.

Look at Scaling 2: Which morphologies are associated/located close by to each complexity metric?
VRM/Area_3d/Rug=  Softcoral plating, live_encr_columnar, Live branching, live submassive, dead_branching, Live collumnar, live porities, Live_Digitate (?)
Pro/Plan Curv= Rubble, Sand
Average Curvature= Dead_Massive, Dead_Collumnar, others?

```{r percent cover rda scaling 2 plot, echo=FALSE}
plot(dig.rda1, scaling = 2, main = "Triplot RDA of dig.percent cover ~ complexity - scaling 2")
spe.sc <- scores(dig.rda1, choices = 1:2, scaling = 2, display = "sp")
...
#### Scaling 2
```{r RDA percent cover scaling 2, include=FALSE}
#Load the data
dig <- read.csv("~/Desktop/GitHub/Bruce_MPQ_Analysis/Data/BaumLab_Digitization_Wide_PercentCover_Data_Clean.csv", header = TRUE, row.names = 1)
...
```{r rda percent cover scaling 2 plot, echo=FALSE}
plot(dig.rda1, scaling = 2, main = "Triplot RDA of dig.percent cover ~ complexity  - scaling 2")
...
#### Scaling 3
```{r RDA percent cover scaling 3, include=FALSE}
#Load the data
dig <- read.csv("~/Desktop/GitHub/Bruce_MPQ_Analysis/Data/BaumLab_Digitization_Wide_PercentCover_Data_Clean.csv", header = TRUE, row.names = 1)
...
```{r RDA percent cover scaling 3 plot, echo=FALSE}
...
### 1.b Percent Cover with fractal dimension

- For this data, since it is only 1 single parameter being examined, multivariate analyses are most likely not the route to go. However, in the essence of staying consistent I did a single RDA for fractal dimension

- R2 value is low, at 0.06 (6%)

- 1st and only RDA axis is significant

```{r Fractal dimension RDA percent cover scaling 1, include=FALSE}
#Load the data
dig <- read.csv("~/Desktop/GitHub/Bruce_MPQ_Analysis/Data/BaumLab_Digitization_Wide_PercentCover_Data_Clean.csv", header = TRUE, row.names = 1)
...
#Ensure data are in dataframes
str(frac) #The environmental data
str(dig) #The abundance data

#Remove non-numeric collumns of dataframes
#Change environment & community to a numeric value only df
com.1b <- dig[ , 7:33]
env.1b <- subset(frac, year == 2015 | year == 2017 | year == 2019)
env.1b <- subset(env.1b, select = c("fractal.dimension"))

nrow(com.1b)
nrow(env.1b)
#Transform the percent cover datasets using the hellinger transformation
dig.hel1b <- decostand(com.1b, "hellinger")

#Conduct RDA
dig.rda1b <- rda(dig.hel1b ~ ., env.1b)
summary(dig.rda1b)

#Obtaining Adjusted R2 value
R2adj.1 <- RsquareAdj(dig.rda1b)$adj.r.squared
R2adj.1 #0.062

# Permutation Test of the RDA
anova.cca(dig.rda1b, step = 1000) #Global test of the RDA results
anova.cca(dig.rda1b, by = "axis", step = 1000) #Tests of all canonical axes
...
#### Scaling 1
```{r fractal dimension percent cover rda scaling 1 plot, echo=FALSE}
plot(dig.rda1b, scaling = 1, main = "Triplot RDA of dig.percent cover ~ complexity - scaling 1")
spe.sc1b <- scores(dig.rda1b, choices = 1:2, scaling = 1, display = "sp")
arrows(0, 0, spe.sc1b[,1], spe.sc1b[,2], length = 0, lty = 1, col = "red") 

```


### 2. Colony Abundance {.tabset}
...
#### Scaling 1
```{r colony count scaling 1, include=FALSE}
dig.col <- read.csv("~/Desktop/GitHub/Bruce_MPQ_Analysis/Data/BaumLab_Digitization_Wide_ColonyCount_Data_Clean.csv", header = TRUE, row.names = 1)
...
```{r colony count scaling 1 plot, echo=FALSE}
...
#### Scaling 2
```{r RDA colony count scaling 2, include=FALSE}
dig.col <- read.csv("~/Desktop/GitHub/Bruce_MPQ_Analysis/Data/BaumLab_Digitization_Wide_ColonyCount_Data_Clean.csv", header = TRUE, row.names = 1)
...
```{r colony count scaling 2 plot, echo=FALSE}
...
#### Scaling 3
```{r RDA colony count scaling 3, include=FALSE}
dig.col <- read.csv("~/Desktop/GitHub/Bruce_MPQ_Analysis/Data/BaumLab_Digitization_Wide_ColonyCount_Data_Clean.csv", header = TRUE, row.names = 1)
...
```{r colony count scaling 3, echo=FALSE}
...
## Extra NMDS code from Morgan {.tabset}
...
#### PLOTTING IN BASE R ####
#Dataframes to use
#Long format of dig.simple dataframe
dig.simple <- read.csv("BaumLab_Digitization_Simple_Long_Cover_Data_Clean.csv", row.names = 1)

dig.simple
#Change environment & community to a numeric value only df
com1 <- dig.simple[ ,8:10]
env1 <- mpq.1[,9:14]

#Standardize mpq data to Z scores
# Standardize predictor variables to Z scores
mpq.z <- env1
mpq.z$rug <- rescale(mpq.z$rug)
mpq.z$vrm <- rescale(mpq.z$vrm)
...
mpq.z <- mpq.z + 1.55 #Add a constant to allow NMDS to work
...
mpq.nmds <- metaMDS(mpq.z, distance = "bray")
mpq.nmds

mpq.bray <- vegdist(mpq.z)
mpq.nmds$stress

plot(mpq.nmds, type = "t", main = paste("NMDS/Bray - Stress=", round(mpq.nmds$stress,3)))


#Test to see the goodness of fit
par(mfrow=c(1,2))
stressplot(mpq.nmds, main = "Shepard Plot")
gof = goodness(mpq.nmds)
plot(mpq.nmds, type = "t", main = "Goodness of fit")
points(mpq.nmds, display = "sites", cex = gof*200)


spe.bray.ward <- hclust(mpq.bray, "ward")
spe.bw.groups <- cutree(spe.bray.ward, k=4)
grp.lev <- levels(factor(spe.bw.groups))


par(mfrow=c(1,1))
sit.sc <- scores(mpq.nmds)
...
#ordiellipse(mpq.nmds, groups = spe.bw.groups, draw = "polygon", lty = 1, col = "grey90")
ordihull(mpq.nmds, groups = spe.bw.groups, draw = "polygon", lty = 1, col = "grey90")


#Add vectors for environmental data to the NMDS plot
env.nmds <- metaMDS(com1, distance = "bray")
env.bray <- vegdist(env2)
env.bray.ward <- hclust(spe.bray, "ward")
env.bw.groups <- cutree(spe.bray.ward, k=4)
...
#Run MDS and envfit functions
dig.pc.mds <- metaMDS(mpq.z, distance = "bray")
...
#Determine the significant the different variables measured
dig.spp.fit #Benthic Morphologies all significant (soft coral, hard coral, macroalage all significant)

#To plot the output from the mds using ggplot a new datasheet needs to be created which contains the x,y points for each site. You can do this by calling the scores of you mds.
site.scrs <- as.data.frame(scores(dig.pc.mds, display = "sites")) #save NMDS results into dataframe
#site.scrs <- cbind(site.scrs, morphology = dig.simple.per.cover$morphology) #add grouping variable "Management" to dataframe
#site.scrs <- cbind(site.scrs, year = dig.simple.per.cover$year) #add grouping variable of cluster grouping to dataframe
#site.scrs <- cbind(site.scrs, Site = rownames(site.scrs)) #add site names as variable if you want to display on plot
...
#This is not necessary if you don’t want to show the species on the final graph.
spp.scrs <- as.data.frame(scores(dig.spp.fit, display = "vectors")) #save species intrinsic values into dataframe
spp.scrs <- cbind(spp.scrs, status = rownames(spp.scrs)) #add species names to dataframe
spp.scrs <- cbind(spp.scrs, pval = dig.spp.fit$vectors$pvals) #add pvalues to dataframe so you can select species which are significant
#spp.scrs<- cbind(spp.scrs, abrev = abbreviate(spp.scrs$Species, minlength = 6)) #abbreviate species names
sig.spp.scrs <- subset(spp.scrs, pval<=0.05) #subset data to show species significant at 0.05
head(spp.scrs)
