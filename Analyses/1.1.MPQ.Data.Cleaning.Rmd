...
#Reorder dataframe
slope1 <- subset(slope, select = c("year", "hum_dist", "sqrt.hdist.cont","pub.site", "site", "ppq", "island.side", "region.3", "region.4", "region.full", "fractal.dimension"))
...
#ReOrder dataframe
colnames(mpq)
mpq <- subset(mpq, select = c("year", "pub.site",  "site", "ppq", "hum_dist", "hdist.cont", "sqrt.hdist.cont", "island.side", "region.3", "region.full", "DEM", "year.site.ppq", "area_2d", "area_3d", "rug", "vrm", "vrm.range", "curv", "pro.curv", "pro.range", "plan.curv", "plan.range"))
...
mpq.3yrs.4cm <- subset(mpq.3yrs, DEM == 4)
mpq.3yrs.8cm <- subset(mpq.3yrs, DEM == 8)
dig_simple_per_w  #Coarse Digitization dataframe
dig_per_w         #Full Dataframe percent coverage
dig.num.w1        #Full Dataframe colony count

#1cm
mpq.dig.1cm <- cbind(mpq.3yrs.1cm,dig_simple_per_w)
mpq.dig.1cm <-mpq.dig.1cm[,-c(22:27)] #remove rows 21-27
mpq.dig.1cm <-mpq.dig.1cm[,-c(24:27)] #remove rows NonCoral, Rubble, sand % cover

#Now bind in full dataframe to this one
mpq.dig.1cm <- cbind(mpq.dig.1cm,dig_per_w) #Percent coverage wide)
mpq.dig.1cm <-mpq.dig.1cm[,-c(25:30)] #remove same metadata already present

#Finally, add in colony count data
mpq.dig.1cm <- cbind(mpq.dig.1cm,dig.num.w1) #Colony Count Data)
ncol(mpq.dig.1cm)
head(mpq.dig.1cm)
mpq.dig.1cm <-mpq.dig.1cm[,-c(52:57)] #remove same metadata already present

#Write data
write.csv(mpq.dig.1cm, "~/Desktop/GitHub/Bruce_MPQ_Analysis/Data/BaumLab_Complex+Digitization_1cm_Data_11Dec20.csv")


#<------ 4cm -------->
mpq.dig.4cm <- cbind(mpq.3yrs.4cm,dig_simple_per_w)
mpq.dig.4cm <-mpq.dig.4cm[,-c(22:27)] #remove rows 21-27
mpq.dig.4cm <-mpq.dig.4cm[,-c(24:27)] #remove rows NonCoral, Rubble, sand % cover

#Now bind in full dataframe to this one
mpq.dig.4cm <- cbind(mpq.dig.4cm,dig_per_w) #Percent coverage wide)
mpq.dig.4cm <-mpq.dig.4cm[,-c(25:30)] #remove same metadata already present

#Finally, add in colony count data
mpq.dig.4cm <- cbind(mpq.dig.4cm,dig.num.w1) #Colony Count Data)
ncol(mpq.dig.4cm)
head(mpq.dig.4cm)
mpq.dig.4cm <-mpq.dig.4cm[,-c(52:57)] #remove same metadata already present

#Write
write.csv(mpq.dig.4cm, "~/Desktop/GitHub/Bruce_MPQ_Analysis/Data/BaumLab_Complex+Digitization_4cm_Data_11Dec20.csv")

#<------ 8cm -------->
mpq.dig.8cm <- cbind(mpq.3yrs.8cm,dig_simple_per_w)
mpq.dig.8cm <-mpq.dig.8cm[,-c(22:27)] #remove rows 21-27
mpq.dig.8cm <-mpq.dig.8cm[,-c(24:27)] #remove rows NonCoral, Rubble, sand % cover

#Now bind in full dataframe to this one
mpq.dig.8cm <- cbind(mpq.dig.8cm,dig_per_w) #Percent coverage wide)
mpq.dig.8cm <-mpq.dig.8cm[,-c(25:30)] #remove same metadata already present

#Finally, add in colony count data
mpq.dig.8cm <- cbind(mpq.dig.8cm,dig.num.w1) #Colony Count Data)
ncol(mpq.dig.8cm)
head(mpq.dig.8cm)
mpq.dig.8cm <-mpq.dig.8cm[,-c(52:57)] #remove same metadata already present

#Write
write.csv(mpq.dig.8cm, "~/Desktop/GitHub/Bruce_MPQ_Analysis/Data/BaumLab_Complex+Digitization_8cm_Data_11Dec20.csv")
...
#Summarize fish counts by site.transect


#Summarize all fish counts by site.spp
ki.fish.sum <- ki.fish %>% group_by(year, site, transect, CoarseFG, year.site.transect) %>% summarise(Total = sum(count))
...
### Add functional group data
ki_full$CoarseFG <- as.character(cred.allo$CoarseFG[match(ki_full$Species, cred.allo$CREDSpecies)])
ki_full$FineFG <- as.character(cred.allo$FineFG[match(ki_full$Species, trophic$CREDSpecies)])
