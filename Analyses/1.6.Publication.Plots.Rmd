...
### a. REALLY old DHW figure (from thesis- replace with copy C here)
```{r dhw temp figure to add to map multipanel}
# Load necessary packages
library(imager)
...
### b. (OLD- text size still too small- Map figure option with CEX 1.5 text)
```{r dhw temp figure to add to map multipanel}
# Load necessary packages
library(imager)
...
### c. KEEP THIS VERSION (Map figure option with CEX 2 text for axis labels)
```{r dhw temp figure to add to map multipanel}
# Load necessary packages
library(imager)
...
```{r Metrics by year with n listed for each timepoint}
#VRM
# p.1 <- ggplot(mpq.1cm, aes(x = year, y=vrm, fill= year)) + geom_boxplot() + theme_classic(base_size = 16) + labs(x="", y= "VRM") +  
#   scale_y_continuous(breaks = c(0.02, 0.05, 0.08, 0.11), limits = c(0.02, 0.12)) +
#  
...
## Figure 4: Regional differences through timepoints (Don't worry about it too much just yet...)
