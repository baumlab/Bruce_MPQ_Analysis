...
```{r MPQ Data from all 5 timepoints (3 sites), echo =FALSE}
#Load the data
mpq.5.tpts <- read.csv(here("Data","MPQ.Complexity.Data.5.tpts.csv"))

#Change the collumn names
colnames(mpq.5.tpts)[which(names(mpq.5.tpts)=="plot")] <- "ppq"
colnames(mpq.5.tpts)[which(names(mpq.5.tpts)=="DEM.scale")] <- "DEM"
colnames(mpq.5.tpts)[which(names(mpq.5.tpts)=="Complexity.Calculation")] <- "rug"
colnames(mpq.5.tpts)[which(names(mpq.5.tpts)=="Avg..BTM_Slope")] <- "slope"
colnames(mpq.5.tpts)[which(names(mpq.5.tpts)=="Avg_terrain_ruggedness..VRM."  )] <- "vrm"
colnames(mpq.5.tpts)[which(names(mpq.5.tpts)=="profile.curvature")] <- "pro.curv"
colnames(mpq.5.tpts)[which(names(mpq.5.tpts)=="planform.curvature")] <- "plan.curv"
colnames(mpq.5.tpts)[which(names(mpq.5.tpts)=="average.curvature")] <- "curv"

#Create new dataframe pulling over only collumms I need
mpq.5.tps<- subset(mpq.5.tpts, select = c("year", "hum_dist","site", "ppq", "SHAPE_Area", "rug", "slope", "vrm", "DEM", "pro.curv", "plan.curv"))

#Switch it back to mpq
mpq.5.tpts <- mpq.5.tps
str(mpq.5.tpts)

#Change DEM, Site, Year,Plot all to character
mpq.5.tpts$DEM <- as.character(mpq.5.tpts$DEM)
mpq.5.tpts$site <- as.character(mpq.5.tpts$site)
mpq.5.tpts$year <- as.character(mpq.5.tpts$year)
mpq.5.tpts$ppq <- as.character(mpq.5.tpts$ppq)

str(mpq.5.tpts)


#Change levels of hum_dist & site
levels(mpq.5.tpts$hum_dist)
mpq.5.tpts$hum_dist <- factor(mpq.5.tpts$hum_dist, levels = c("Very High", "Medium", "Very Low" ))
mpq.5.tpts$site <- factor(mpq.5.tpts$site, levels =c("27", "30", "32", "8", "34", "35", "5", "15", "19", "37"))

##Drop rows where values = NA
mpq.5.tpts %>% filter(!is.na(hum_dist))
...
#Load mpq data
#mpq <- read.csv(here("Data","ArcMap.Complexity.Data.Final.csv"))
mpq <- read.csv(here("Data","MPQ.Complexity.2015.2017.2019.allDEM.27Aug2020.csv"))
mpq
str(mpq)
colnames(mpq)
...
#Make a subset for 4cm DEM 
mpq.4 <- mpq[mpq$DEM == 4,] 

#Reorder sites so they fit in as VH --> M --> VL
mpq.4$site <- factor(mpq.4$site, levels = c("27", "30", "32", "8", "34", "35", "5", "15", "19", "37"))
...
#4cm DEM
levels(mpq.4$site.plot)
mpq.4$site.ppq <- factor(mpq.1$site.ppq, levels=c("27.1", "27.2", "27.3", "30.1", "30.2", "30.3", "32.1", "32.2", "32.3", "8.1", "8.2", "8.3", "34.1", "34.2", "34.3", "5.1", "5.2", "5.3", "15.1", "15.2", "15.3", "19.1", "19.2", "19.3", "37.1", "37.2", "37.3"))
levels(mpq.4$site.ppq)

#8cm DEM
levels(mpq.8$site.plot)
...
#loading the data
dig <- read_excel(here("Data", "MPQ.Digitization.Data.27Jul2020.xlsx")) #this is a new dataframe, so to fix everything just change this
...
levels(dig$year.site.ppq)
dig$year.site.ppq <- factor(dig$year.site.ppq, levels = c("27.1", "27.2", "27.3", "30.1", "30.2", "30.3", "32.1", "32.2", "32.3", "8.1", "8.2", "8.3", "34.1","34.2","34.3", "35.1","35.2","35.3", "5.1","5.2","5.3", "15.1", "15.2", "15.3", "19.1", "19.2"))
levels(dig$year.site.ppq)
...
dig_SR <- subset(dig, select = c("year", "hum_dist", "site", "ppq", "Benthic_Morphology", "area", "SArea", "surface.rugosity"))
...
dig_SR_sum <- aggregate( surface.rugosity ~ Benthic_Morphology, dig_SR, mean )
...
# ArcMap Complexity metrics PCoA
...
SR1 <- ggplot(mpq.1, aes(x = year, y=rug)) + geom_boxplot(fill="deepskyblue") + theme_classic(base_size = 10) +facet_wrap(~DEM, scales = "fixed", ncol= 2)+ xlab("Year") + ylab("Surface Rugosity") + geom_label(label= "p=3.94E-3", x= 2, y=2.9, label.size= 0.02, label.padding= unit(0.02, "lines"), fill= "grey", color="black") +
  ylim(1, 3.0)

SR2 <- ggplot(mpq.4, aes(x = year, y=rug)) + geom_boxplot(fill="red") + theme_classic(base_size = 10) +facet_wrap(~DEM, scales = "fixed", ncol= 2)+ xlab("Year") + geom_label(label= "p=0.213", x= 2, y=2.9, label.size= 0.02, label.padding= unit(0.02, "lines"), fill= "grey", color="black")+
  ylim(1, 3)+ ylab("") 


SR3 <- ggplot(mpq.8, aes(x = year, y=rug)) + geom_boxplot(fill="forestgreen") + theme_classic(base_size = 10) +facet_wrap(~DEM, scales = "fixed", ncol= 2)+ xlab("Year") + geom_label(label= "p=0.455", x= 2, y=2.9, label.size= 0.02, label.padding= unit(0.02, "lines"), fill= "grey", color="black")+
  ylim(1, 3)+ ylab("") 
...
TR1 <- ggplot(mpq.1, aes(x = year, y=vrm)) + geom_boxplot(fill="deepskyblue") + theme_classic(base_size = 10) +facet_wrap(~DEM, scales = "fixed", ncol= 2)+ xlab("Year") + ylab("Terrain Ruggedness") + geom_label(label= "p=5.83E-7", x= 2, y=0.13, label.size= 0.2, label.padding= unit(0.2, "lines"), fill= "grey", color="black") +
  ylim(0, 0.14)

TR2 <- ggplot(mpq.4, aes(x = year, y=vrm)) + geom_boxplot(fill="red") + theme_classic(base_size = 10) +facet_wrap(~DEM, scales = "fixed", ncol= 2)+ xlab("Year") + ylab("") + geom_label(label= "p=0.0583", x= 2, y=0.13, label.size= 0.2, label.padding= unit(0.2, "lines"), fill= "grey", color="black")+
  ylim(0, 0.14)


TR3 <-ggplot(mpq.8, aes(x = year, y=vrm)) + geom_boxplot(fill="forestgreen") + theme_classic(base_size = 10) +facet_wrap(~DEM, scales = "fixed", ncol= 2)+ xlab("Year") + ylab("") +geom_label(label= "p=0.306", x= 2, y=0.13, label.size= 0.2, label.padding= unit(0.2, "lines"), fill= "grey", color="black")+
  ylim(0, 0.14)
...
S1 <- ggplot(mpq.1, aes(x = year, y=slope)) + geom_boxplot(fill="deepskyblue") + theme_classic(base_size = 10) +facet_wrap(~DEM, scales = "fixed", ncol= 2)+ xlab("Year") + ylab("Slope") +geom_label(label= "p=0.0237", x= 3, y=48, label.size= 0.2, label.padding= unit(0.2, "lines"), fill= "grey", color="black") +
  ylim(10, 50)

S2 <- ggplot(mpq.4, aes(x = year, y=slope)) + geom_boxplot(fill="red") + theme_classic(base_size = 10) +facet_wrap(~DEM, scales = "fixed", ncol= 2)+ xlab("Year") + ylab("") +geom_label(label= "p=0.477", x= 3, y=48, label.size= 0.2, label.padding= unit(0.2, "lines"), fill= "grey", color="black") +
  ylim(10, 50)


S3<- ggplot(mpq.8, aes(x = year, y=slope)) + geom_boxplot(fill="forestgreen") + theme_classic(base_size = 10) +facet_wrap(~DEM, scales = "fixed", ncol= 2)+ xlab("Year") + ylab("") +geom_label(label= "p=0.586", x= 3, y=48, label.size= 0.2, label.padding= unit(0.2, "lines"), fill= "grey", color="black") +
  ylim(10, 50)
...
Pro2 <- ggplot(mpq.4, aes(x = year, y=pro.curv)) + geom_boxplot(fill="red") + theme_classic(base_size = 10) +facet_wrap(~DEM, scales = "fixed", ncol= 2)+ xlab("Year") + ylab("P")+geom_label(label= "p=0.0163", x= 2, y=400, label.size= 0.2, label.padding= unit(0.2, "lines"), fill= "grey", color="black")+
  ylim(500, -500)


Pro3 <- ggplot(mpq.8, aes(x = year, y=pro.curv)) + geom_boxplot(fill="forestgreen") + theme_classic(base_size = 10) +facet_wrap(~DEM, scales = "fixed", ncol= 2)+ xlab("Year") + ylab("")+geom_label(label= "p=0.137", x= 2, y=400, label.size= 0.2, label.padding= unit(0.2, "lines"), fill= "grey", color="black")+
  ylim(500, -500)
...
Plan1 <- ggplot(mpq.1, aes(x = year, y=plan.curv)) + geom_boxplot(fill="deepskyblue") + theme_classic(base_size = 10) +facet_wrap(~DEM, scales = "fixed", ncol= 2)+ xlab("Year") + ylab("Planform Curvature")+geom_label(label= "p=8.65E-5", x= 2, y=400, label.size= 0.2, label.padding= unit(0.2, "lines"), fill= "grey", color="black") + ylim(500, -500)

Plan2 <- ggplot(mpq.4, aes(x = year, y=plan.curv)) + geom_boxplot(fill="red") + theme_classic(base_size = 10) +facet_wrap(~DEM, scales = "fixed", ncol= 2)+ xlab("Year") + ylab("")+geom_label(label= "p=0.00872", x= 2, y=400, label.size= 0.2, label.padding= unit(0.2, "lines"), fill= "grey", color="black")+ ylim(500, -500)

Plan3 <- ggplot(mpq.8, aes(x = year, y=plan.curv)) + geom_boxplot(fill="forestgreen") + theme_classic(base_size = 10) +facet_wrap(~DEM, scales = "fixed", ncol= 2)+ xlab("Year") + ylab("")+geom_label(label= "p=0.0244", x= 2, y=400, label.size= 0.2, label.padding= unit(0.2, "lines"), fill= "grey", color="black")+ ylim(500, -500)
...
ggplot(mpq, aes(x = site, y=rug, fill = DEM)) +facet_wrap(~year, scales = "fixed", ncol= 3) + ggtitle("All DEM scales") + geom_boxplot()+ theme_classic(base_size = 14) + xlab("Site") + ylab("Surface Rugosity")+ theme(plot.title = element_text(hjust = 0.5))+ theme(plot.title = element_text(size=22))
...
ggplot(mpq.4, aes(x = site, y=rug, fill = year)) + ggtitle("4cm DEM scale") + geom_boxplot()+ theme_classic(base_size = 14) + xlab("Site") + ylab("Surface Rugosity")+ theme(plot.title = element_text(hjust = 0.5))+ theme(plot.title = element_text(size=22))
...
ggplot(mpq.4, aes(ppq, rug, colour = year)) + geom_boxplot() +facet_wrap(~site, scales = "fixed", ncol= 3) + xlab("PPQ") + ylab("Surface Rugosity")+ theme(plot.title = element_text(hjust = 0.5))+ theme(plot.title = element_text(size=22)) + ggtitle("1cm DEM")
...
### Surface Rugosity Values by morphology
...
ggplot(dig, aes(x=Benthic_Morphology, y=surface.rugosity)) + geom_boxplot(fill="deepskyblue")+ theme(axis.text.x= element_text(angle = 290)) #+ theme_classic(base_size = 10) + xlab("Timepoint") + ylab("Volume Change (m3)")+ theme(plot.title = element_text(hjust = 0.5))+ theme(plot.title = element_text(size=14))+ theme(plot.title = element_text(size=22))+ geom_hline(yintercept = 0, linetype="dashed", color="red", size=0.2) + scale_y_continuous(breaks=seq(-2.5,1,0.2)) + geom_label(label= "p=0.522", x= 3, y=0.9, label.size= 0.5, label.padding= unit(0.5, "lines"), fill= "grey", color="black")
...
# ArcMap Complexity at 5 Timepoints (3 Sites)

- Complexity Metrics quantified from all 5 timepoints (May 2015, March 2016a, November 2016b, July 2017, July 2019) from 3 sites, one from each human disturbance level:
- VL: Site 5
- M: Site 8
- VH: Site 27
...
- High variability among the years meant that none years were significantly different from one another after an ANOVA

```{r mpq complex 5 timepoints @ 3 sites, echo=FALSE}
#Plot results
ggplot(mpq.5.tpts, aes(x = year, y=rug)) + geom_boxplot(fill="deepskyblue") + theme_classic(base_size = 10) + xlab("Year") + ylab("Surface Rugosity") + geom_label(label= "p=0.202", x= 4, y=2.3, label.size= 0.02, label.padding= unit(0.02, "lines"), fill= "grey", color="black")

# a20 <- aov(rug~year, data = mpq.5.tpts)
# summary(a20)
#TukeyHSD(a20)
...
- Similar high variability is seen to that in surface rugosity, however significant results are found after Anova/Tukey tests

```{r vrm 5tpt, echo=FALSE}
ggplot(mpq.5.tpts, aes(x = year, y=vrm)) + geom_boxplot(fill="deepskyblue") + theme_classic(base_size = 10) + xlab("Year") + ylab("Terrain Ruggedness") + geom_label(label= "p=9.27E-4", x= 4, y=0.08, label.size= 0.02, label.padding= unit(0.02, "lines"), fill= "grey", color="black")

# a21 <- aov(vrm~year, data = mpq.5.tpts)
# summary(a21)
# TukeyHSD(a21)
```
...
```{r Profile Curvature 5ptp, echo=FALSE}
ggplot(mpq.5.tpts, aes(x = year, y=pro.curv)) + geom_boxplot(fill="deepskyblue") + theme_classic(base_size = 10) + xlab("Year") + ylab("Profile Curvature")

# a22 <- aov(pro.curv~year, data = mpq.5.tpts)
# summary(a22)
# TukeyHSD(a22)
```


```{r procurv 5 tpts, echo=FALSE}


ggplot(mpq.5.tpts, aes(x = year, y=plan.curv)) + geom_boxplot(fill="deepskyblue") + theme_classic(base_size = 10) + xlab("Year") + ylab("Planform Curvature") #+ geom_label(label= "p=3.94E-3", x= 2, y=2.9, label.size= 0.02, label.padding= unit(0.02, "lines"), fill= "grey", color="black") +ylim(1, 3.0)

# a23 <- aov(plan.curv~year, data = mpq.5.tpts)
# summary(a23)
# TukeyHSD(a23)
...
# At the human disturbace/site level

- As there is only 1 site per human disturbance of this dataframe, these plots are equivalent to those at the site level too

#### Surface Rugosity
- We see a visual difference in ithe values seen across the disturbance gradient, however the trends varied at each site depending on the complexity metric sampled

```{r 5tpts by hum_dist, echo=FALSE}
#By Human Disturbance
ggplot(mpq.5.tpts, aes(x = year, y=rug)) + geom_boxplot(fill="deepskyblue") + facet_wrap(~hum_dist, scales = "fixed", ncol= 3)+ theme_classic(base_size = 10) + xlab("Year") + ylab("Surface Rugosity") + geom_label(label= "p=4.33E-11", x= 2, y=2.9, label.size= 0.02, label.padding= unit(0.02, "lines"), fill= "grey", color="black") +ylim(1, 3.0)

#a24 <- aov(rug~site, data = mpq.5.tpts)
#summary(a24) #P=4.33E-11
#TukeyHSD(a24) #all 3 are significantly different from one another

```

Statistical Results:
- There is a significant difference between all sites 8-27, 5-27, and 5-8


#### Terrain Ruggedness

```{r vrm site 5tpts, echo=FALSE}
ggplot(mpq.5.tpts, aes(x = year, y=vrm)) + geom_boxplot(fill="deepskyblue") + facet_wrap(~hum_dist, scales = "fixed", ncol= 3)+ theme_classic(base_size = 10) + xlab("Year") + ylab("Terrain Ruggedness") + geom_label(label= "p=1.25E-05", x= 3.5, y=0.09, label.size= 0.02, label.padding= unit(0.02, "lines"), fill= "grey", color="black")

#a25 <-aov(vrm~site, data = mpq.5.tpts)
#summary(a25) #P = 1.25E-05
#TukeyHSD(a25) #Sites 5-8 not sig different, all other combos are
```

Statistical Results:
- Significant differences are seen between sites 8-27 and 5-27
- Site 5-8 are not significantly different (P=0.1795679)

#### Profile Curvature

```{r pro.curv site 5tps, echo=FALSE}
ggplot(mpq.5.tpts, aes(x = year, y=pro.curv)) + geom_boxplot(fill="deepskyblue") + facet_wrap(~hum_dist, scales = "fixed", ncol= 3)+ theme_classic(base_size = 10) + xlab("Year") + ylab("Profile Curvature")+ geom_label(label= "p=2.06E-07", x= 3.5, y=400, label.size= 0.02, label.padding= unit(0.02, "lines"), fill= "grey", color="black")

# a26 <-aov(pro.curv~site, data = mpq.5.tpts)
# summary(a26) #P=2.06E-07
# TukeyHSD(a26) #5-8 not sig different, all other combos are

```

Statistical Results:
- Significant differences are seen between sites 8-27 and 5-27
- Site 5-8 are not significantly different (P=0.0751558)


#### Planform Curvature

```{r plan.curv site 5tps, echo=FALSE}
ggplot(mpq.5.tpts, aes(x = year, y=plan.curv)) + geom_boxplot(fill="deepskyblue") + facet_wrap(~hum_dist, scales = "fixed", ncol= 3)+ theme_classic(base_size = 10) + xlab("Year") + ylab("Planform Curvature")+ geom_label(label= "p=3.01E-07", x= 3.5, y=400, label.size= 0.02, label.padding= unit(0.02, "lines"), fill= "grey", color="black")

# a27 <-aov(plan.curv~site, data = mpq.5.tpts)
# summary(a27) #P=3.01E-07
# TukeyHSD(a27) #5-8 not sig different, all other combos are
