...
```{r doms dig df help, include=FALSE}
#loading the data
...
dig$year.site.ppq <- paste(dig$year, dig$site, dig$ppq, sep="_")
...
#calculates the percent area for each Benthic Substrate
...
Morphology.percent.plot <- ggplot(dig_sum, aes(fill=Benthic_Morphology, y=Per_Area, x=year.site.ppq)) + geom_bar(stat = 'identity', position = 'stack') + theme_DOM() + labs(x="Year Site Quadrat", y="Percent Area (%)")
...
for (i in 1:length(max_benthic$year.site.ppq)) {
  ...
  xx <- subset(dig_sum_extra, year.site.ppq == unique(dig_sum_extra$year.site.ppq)[i] & (Benthic_Morphology == "Rubble" | Benthic_Morphology == "Dead_Massive"))
...
for(i in 1:length(max_benthic$year.site.ppq)){
  ...
  max_benth_var <- max_benth$max_benthic
  
  combo_var <- paste(max_benth_var, "Live_Encrusting", sep = "n")
...
#Plotting the percent area for each benthic substrate in each year site ppq combination
Morphology.extra.percent.plot <- ggplot(dig_sum_removed_full, aes(fill=Benthic_Morphology, y=Per_Area, x=year.site.ppq)) + geom_bar(stat = 'identity', position = 'stack') + theme_DOM() + labs(x="Year Site Quadrat", y="Percent Area (%)")
